<template>
  ...
  <FormPage title="Team Selection" ref="page">
    ...
    <FormGroup :label-type="LabelType.LabelTag" id="match-level-input" name="Match Level">
      <select id="match-level-input" v-model.number="matchLevel" :disabled="config.data.forceQualifiers" style="font-family: 'MartelSans'">
...
function loadTBAData() {
  tba.load("2024cthart", "teams").then(value => updateStatus($$(teamsLoadStatus), $$(teams), value));
  tba.load("2024cthart", "matches").then(value => updateStatus($$(matchesLoadStatus), $$(matches), value));
}
</script>
